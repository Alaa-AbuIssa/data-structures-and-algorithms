...
//   let positionIndex = arr.map(result => result.length).indexOf(Math.max(...arr.map(result => result.length)));
...
//   let newArray = arr.filter(result => result.includes(':)'));
...
//   // Solution code here...
//   let phoneNumberArray = arr.map(result => result.substring(1, 4) + result.substring(6, 9) + result.substring(10, 14));
...
//   // Solution code here...
//   let oddIndex = str.split('').reduce((acc, val, idx) => (idx % 2 !== 0) ? acc += val : acc, '');
//   return oddIndex;
// };
...
// const allHappy = (arr) => {
//   // Solution code here...
//   let x = ((arr.filter(result => !(result.includes(':)'))).length > 0) ? false : true);
//   return x;
// };
